...
该模式需要由主控设备输入指令来开启或关闭算法，在串口模式或I2C模式下，读写寄存器来实现算法的开启或关闭，我们在多个编程平台提供了封装好的库共用户使用

串口模式需要结合“标准协议指令”或“简单协议指令”，详见相关章节介绍

I2C模式需要直接读写寄存器

算法开启流程如下：
    
1. 向寄存器0x20-VISION_ID写入要开启的算法ID号
    
2. 向寄存器0x21-VISIO_CONF1中写入0x01,即可开启算法
    
3. 向寄存器0x21-VISIO_CONF1中写入0x00,即可关闭算法
