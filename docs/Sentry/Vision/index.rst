...
* 算法简介
    ...
    对识别区域内像素的颜色信息进行统计分析，判断出该区域最可能的颜色分类。每个检测位置称为一个识别区域，其位置和大小由用户进行设置，最多25组识别区域，返回该区域的颜色标签信息和实际的红R、绿G、蓝B数值。
...
* 配置参数
    ...
    在设置页面里，有几种预置的识别区域网格分布形式和识别区域大小：
