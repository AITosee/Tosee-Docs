...
当使用串口设备与主控进行数据交互时，需要采用一定的交互协议才可以完成数据或指令的传输。Sentry系列视觉传感器支持 :ref:`标准指令协议<chapter_standard_protocol_index>` 和 :ref:`简单指令协议<chapter_simple_protocol_index>`
...
标准指令协议具有规范的数据格式，结合寄存器表可以实现完整的数据交互功能和较高的通信效率。
